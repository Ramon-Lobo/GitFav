...
export function* addFavoriteRequest(action) {
  try {
    ...
    if (favorites.find(favorite => favorite.id === response.data.id)) {
      yield put(FavoriteActions.addFavoriteError("Repository is already in your favorites"));
    } else {
      yield put(FavoriteActions.addFavoriteSuccess(response.data));
    }
  } catch (err) {
    yield put(FavoriteActions.addFavoriteError("This repository doesn't exist"));
  }
}
